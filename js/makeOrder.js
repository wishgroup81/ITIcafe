...
if (user_login) {
  ...
  function selectProduct() {
    ...
    fetch(
      `http://localhost/makeOrder.php?product-id=${id}&user-id=${user_login}`
    ).then((data) => {
      if (data) {
        console.log(id);
  ...
  function selectProduct() {
    const id = this.dataset.productId;
    fetch(
      `http://localhost/adminmakeOrder.php?product-id=${id}&admin-id=${admin_login}`
    ).then((data) => {
      if (data) {
        window.location.reload();
      }
...
function cancelProduct() {
  ...
  console.log(id);
  fetch(`http://localhost/makeOrder.php?cancel-id=${id}`).then((data) => {
    if (data) {
