...
function selectProduct() {
  ...
  fetch(`http://localhost/cafteria/makeOrder.php?product-id=${id}&user-id=${user_login}`)
    .then((data) => {
      if (data) {
...
function selectProduct() {
  const id = this.dataset.productId;
  fetch(`http://localhost/cafteria/makeOrder.php?product-id=${id}&admin-id=${admin_login}`)
    .then((data) => {
      if (data) {
        window.location.reload();
      }
...
function cancelProduct() {
  ...
  console.log(id);
  fetch(`http://localhost/cafteria/makeOrder.php?cancel-id=${id}`).then(
    (data) => {
      if (data) {
...
function increaseAmount() {
  ...
  fetch(`http://localhost/cafteria/makeOrder.php?increase-id=${id}`).then(
    (data) => {
      if (data) {
...
function decreaseAmount() {
  ...
  fetch(
    `http://localhost/cafteria/makeOrder.php?decrease-id=${id}&amount=`
  ).then((data) => {
    if (data) {
